...
The request (the user's optional argument): the request as given in the message that invoked this skill.
...
3. **Where recovered rationale goes.** If the pass runs, some candidate ledger items become *provisionally attributable* — a commit body appears to explain the *why*. Attribution is what can keep a choice **off** the ledger (the ledger is the *un*attributed set), but it is only ever a **proposal until the operator accepts it** at the Phase 3 architecture gate: a commit body can be stale, superseded, or describe a different past state than the code's current one. So a git find does not remove the item now — it is carried into Phase 3 as a *pending* attribution and presented for confirmation. An attributed choice must also not silently vanish: unlike a *dismiss*, a git find carries real recovered information worth keeping. On the operator's decision at the gate:
   ...
   **When one is promoted, the ADR records ratification, not choice.** Pass the commit SHA as the cited evidence and say the operator ratified it at this gate; `/b-adr` writes it as *"Observed in `a1b2c3d`; the operator ratified."* It must never read *"the operator chose X"* — nobody put options to them, and the weighing, if it happened at all, happened in someone else's head at an unknown past moment. *Ratified* is earned here and almost nowhere else in the framework, because this gate is a genuine per-item accept / reject-as-stale / reject-as-wrong decision against a citable, durable artefact.
...
- **All three drain paths remove the line.** Resolve → the intent is recovered *from the operator*, now that there is a concrete reason to ask, and captured as an ADR via `/b-adr`, **which deletes the line itself**: writing the ADR is the whole of that exit, so nothing is handed back for the operator to remember, and the ADR is cold and module-scoped, loaded only when that area is touched. Because the intent came from a live operator utterance, the ADR records it as demarcated operator wording — citing the commit instead would understate the provenance. Remediate → the change lands via `/b-feature` (or `/b-design` for an architectural fix), reconciling docs the normal way; delete the line once the fix is in. Dismiss → delete the line (no tombstone in v1 — re-running adoption on an already-adopted project is not a supported operation).
...
3. **Handoff block** — the only end-of-workflow output. Use the phase-open form when N ≥ 1, the clean form when N = 0.
   ...
   **N ≥ 1 (adoption phase open):**
   ...
   /b-adr deletes the line on resolve; the other two exits are yours to delete.
   When the ledger is empty, delete the banner from docs/index.md — /b-adr
   offers to do it when the item it resolved was the last one.
