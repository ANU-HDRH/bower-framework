...
- **A claim annotated *decided, not built* is not drift.** Treat it as non-existent, as under `## Not yet in force`; never report it as spec-code drift (`_bower/review-schema.md` → dimension 2).
- **Report an annotation's lifecycle only where your own inputs settle it** — an annotation you met in this module's feature plans or its `## Software architecture` entry, resolved against this module's `## Build order`. Exactly two cases:
