...
`docs/conformance/` holds eight scenarios, written pass criteria, and an append-only ledger (`runs.md`) recording every run as PASS / PASS-WITH-DEGRADATION / FAIL. It is where the tier claims in the README — Claude Code *supported*, Codex *experimental* — are defined, evidenced, and caveated where the evidence is short, and it is not scaffolded into projects. The scenarios are behavioural: no static check can tell you whether a gate actually stopped.
...
The rule is per-runtime and binds only where the scenario applies — C8 is Codex-only, so a Claude claim is discharged by C3 alone. And it has a floor the wording alone does not reach: **a tier whose evidence set is incomplete must say so where the tier is claimed**, in `docs/conformance/README.md` and in the public README both. Claude Code's `supported` is the live instance — it predates the suite and owes C1, C2, C4 and C5 — and the two claim sites carry that caveat rather than implying rows that do not exist. Do not tidy the caveat away while the rows are still owed; check `runs.md` first.
