...
An optional line `Context: inline` may follow the `Generated by` line. It is written **only by the calling command** (`/b-design`, `/b-analysis`) when the runtime could not delegate and the caller followed the analyst's definition inline — it records that the brief lacks fresh-context isolation. A genuinely delegated analyst never writes this line; absence means the brief was produced in an isolated context.
