...
`plan.md` and `architecture.md` are **descriptive**: they say what the system *is*. That property is the whole reason an agent may read them instead of the code, and it is what the Working Conventions rule *don't search the codebase when the map exists* rests on. The map is only safe while it describes something that exists.

The failure mode is a claim about code that is **decided but not built**. Three commands legitimately write one. `/b-design` records decisions into `architecture.md` and into sibling `plan.md` files whose implementing `/b-feature` may not run for weeks. Those siblings are usually *completed* features, so an annotation routinely sits in a `✓` plan and names a different, unbuilt feature: **the marker on the plan you are reading says nothing about whether its claims are early.** What decides that is which feature will make the claim true. `/b-feature` Step 3 writes its own plan before any code, deliberately, because that file is the recovery anchor, and `/b-module` does the same for each feature in its build loop. In both cases the doc briefly describes a system that is not there.

Written in the present indicative, such a claim is **indistinguishable from a stale one** — and the two resolve in opposite directions. *Code is truth* makes a stale doc the finding and the code correct; it makes an early doc merely early and the doc correct. A reader who cannot tell them apart has to check the code, which is the exact check the doc exists to prevent. Every markered surface already solves this — `⏸` says *planned, not built* and nobody misreads it. Prose has no such vocabulary, so a design run with no way to say "decided" says "built".

**The marker text is invariant — `decided, not built`, what decided it, and the *owner* that will make it true.** What decided it is the ADR where there is one; where an operator gate settled the shape and no ADR was written — a `/b-design` stage gate, or a `/b-feature` or `/b-module` gate — it is `gate YYYY-MM-DD`. Never invent an ADR ID to fill the slot. **The marker is matched case-insensitively, and read together with the two lines after it.** The banner form opens a sentence and so capitalises — `**Decided, not built**` — while the inline form does not, so every sweep is `grep -i`; a case-sensitive one skips precisely the whole-section banners, which are the annotations a design run writes most. And the owner is routinely *not* on the marker's own line: the banner puts what decided it on the first line and the owner on the second, and a formatter re-wraps an inline clause anywhere. So the sweep form is `grep -rinA2` and the owner is read from that three-line window — a bare `grep -rni` returns the marker line alone, which for the canonical banner is the half with the owner missing from it. The viewer reads the same window, stopping early at a second marker so two adjacent annotations cannot borrow each other's fields. **The owner is always introduced by the word `feature` and written in backticks** — `` feature `<module>/<feature>` `` or `` feature `<module>/Q-<slug>` `` — as every example below does. That token is what the viewer reads the owner from; `owner:`, `built by`, or a bare backticked name is an annotation the viewer reports as ownerless. The owner is one of exactly two things, and there is no third:
...
**A queue-item owner is a name two commands would otherwise change under it, so both are told not to.** `Q-<slug>` is stable against concurrent writers by construction — that is why v0.38 made it a name rather than a count — but it is not stable against the two operations that rewrite queue IDs, and an annotation is the first thing that ever pointed at one. `/b-merge` renames one side's slug where two branches took the same name for different drifts; that rename now re-points every annotation naming the old slug, and its standing claim that a queue has no inbound references to repair is retired. `/b-review`'s absorb-at-triage moves an item into the review plan under a fresh `F<n>` and deletes it from the queue, which would strand the annotation at the moment the work started being tracked *better* — so **an item that owns an annotation is not absorbed**, and `/b-review` says why at the triage gate. Leaving it in the queue costs nothing: the queue holds nothing open, and the owner stays where the annotation, the discharge sweeps and the viewer can all still resolve it. There is deliberately no `<module>/F<n>` owner form. A review plan is deleted at closeout, which makes it the least durable of the three files, and a `[~]` won't-fix would carry it off with an annotation still naming it.

Placement follows scope:

- **A section written wholly ahead** carries a blockquote banner directly under its heading — a blockquote, and before anything else in the section. The form is load-bearing rather than decorative: a reader (and the viewer) takes the banner as covering everything below it to the next heading, so an annotation written as ordinary prose part-way down a section covers only the claim it sits in. In a `## Components` table that distinction decides whether one row or the whole table is exempt from `component-missing`.
...
The rules that follow — six, and the first two are what make the convention work at all:

- **Agents treat an annotated claim as non-existent**, exactly as under `## Not yet in force`: do not build on it, do not cite it as the current contract, do not report it as drift. Verify that part against the code and proceed from what is actually there. This is the one exception to *read the plan, don't search the codebase* — and it is an exception the map declares about itself, which is why the convention costs the reader nothing elsewhere.
- **There is no ownerless annotation.** The owner is what makes the annotation removable and auditable: a removal owner needs to know which change discharges it, and the viewer's lifecycle checks read it. An annotation with no owner cannot be discharged and cannot be audited, so after the work lands it is a false claim in the one register every agent has been told to trust — worse than the unannotated prose the convention replaced, because the annotation actively instructs readers to treat built code as absent. **An annotation and its owner are written together or neither is written.** One found in the wild without an owner is a write-side defect, and it gets a **durable record rather than a printed one** — a handoff line dies with the console, which is the same failure the queue was invented to stop. A command that may write the queue offers a findings-queue item naming the file, the line and the claim, at the same gate it offers any other out-of-scope drift: `/b-feature` Step 6.12, `/b-module` Step 5.12, `/b-merge`'s coherence gate. `/b-review` may not — a review never pushes a finding down into the queue (*Findings queue*, rule 6) — so there the record is the review plan: `bower-reviewer` reports it as an observation, because choosing an owner is not a read-only agent's call, and the triage gate puts it to the operator as an acceptable finding rather than something to read past. Neither path adopts the annotation, and neither deletes it. Where a decision implies work no build-order entry will carry — `/b-design`'s implied-not-tracked case, a component reshaped rather than a feature added — the owner is a findings-queue item recorded in the same pass (`## Findings queue`, below); a printed handoff pointer is not an owner, since nothing on disk survives it.
- **The owner name carries its module.** Feature names are *module-scoped by construction* — the directory is `docs/modules/<module>/<feature>/` — so two modules may each hold a `crud` or a `settings`, and a bare name identifies nothing; queue IDs are module-local for the same reason, which is why a routed command that names one also names its path. Qualify always, not only when a clash exists today: an unqualified name that happens to resolve now is silently re-pointed the day another module adds a feature of that name, and the writer cannot see that coming.
- **It is deleted, not updated.** The annotation is history-adjacent living in a current-state doc, so it needs an owner or it becomes the accretion *living documentation* exists to prevent. The owner is the command that builds the thing, and each deletes **as it builds** rather than at the end: `/b-feature` opens Step 6 with the sweep, before any marker, stamp or tick, because the code exists from the moment its tests passed and every annotation the change discharged is false from then on; `/b-module` does the same inside its build loop, per feature at Step 3.7, so a loop that stops mid-module leaves no completed feature's claim behind. Where a discharge depends on a check the run has not yet made — a findings-queue tick — the deletion shares that write instead of preceding it. Only the annotation goes — the claim it wrapped is true now and stays. This is also the one edit `/b-feature`, `/b-module` and `/b-review` may make to `architecture.md`, because `/b-design` writes annotations there and no other command comes back for them; it removes a marker and no claim, which is why it does not trip the architectural hard redirect. Where deleting the annotation would leave the surrounding prose *wrong* rather than merely unmarked, it is an architecture edit and routes to `/b-design`. **Removing the owner inverts the rule.** `/b-feature remove` takes a build-order entry away, and every annotation that entry owned now wraps a claim nothing will ever build — so there the *claim* goes, annotation and all, in whatever plan holds it; in `architecture.md` the remove intent's existing public-surface rule decides whether that is its edit or `/b-design`'s.
- **A greenfield draft is not annotated.** The annotation separates an early claim from a stale one inside a *partially built* project. A greenfield `architecture.md` describes a system where nothing exists and every marker in the project says so, so there is no built claim for an early one to be mistaken for — annotating every paragraph would spend the marker's entire value on a distinction nobody can draw wrongly. `/b-design` annotates on **revision**: an edit into a document that also describes code that exists. From the first built feature onward, that is every design run. **The same exemption covers a new module's `## Software architecture` entry** in a revision — every marker on that module reads `⏸`, so the entry is as visibly unbuilt as a greenfield draft, and no single feature could own it anyway. The exemption is exactly that entry: a runtime-view edit that weaves the new module into a built section, or a claim added to an existing module's entry, is annotated normally and owned by whichever of the new module's features builds the thing.
- **Nothing else is ever annotated.** Not `status.md`, `module-status.md`, `scope.md`, `constitution.md`, `ui.md`, or an ADR body. Each already has its own vocabulary for the not-yet-true — a status marker, `Pending verification:`, `## Not yet in force` — or is immutable. A second convention in those files would be two writers for one fact.

**Pull-forward moves the discharge along with the scope.** When a feature absorbs part of a later entry's scope (*Pull-forward annotation*, above), it may make true a claim whose owner still reads as that later entry — and the discharging sweeps filter on the owner's *name*. Nothing else would catch it: the later entry is still `⏸`, so neither lifecycle check below fires, and the annotation goes on telling every reader that built code does not exist. So a command that writes a pull-forward annotation treats the absorbed entry as an owner it may discharge in the same pass. Where the absorption left `Remaining: none`, every annotation owned by that entry is discharged and its annotation deleted; where something remains, only the annotations covering what actually landed go, and the rest stay for that entry's own pass. The `Remaining: none` half is the one case decidable from `docs/` alone, and the viewer reports it.

**One annotation is self-owned, and it is the only one with a window.** `/b-feature` Step 3 and `/b-module` Step 3.2 write an annotation into the plan of the very feature that owns it, so writer and deleter are the same workflow and the annotation exists for the length of one run. `/b-feature` Step 6.1 and `/b-module` Step 3.7 delete it and write the plan's `Confirmed YYYY-MM-DD` line in the same edit, whatever marker the feature lands on. A self-owned annotation on a built feature is not a resting state and the audits below fire on it, but it has two readings with opposite repairs — a run that finished and skipped the deletion, or a run interrupted after the plan write — and only the code decides; nothing may assert one.

**`Confirmed YYYY-MM-DD` covers the plan's unannotated claims**: each describes code that exists and whose tests have run. An annotated claim is outside it, which is how a `/b-design` annotation sits in a stamped `✓` plan without contradiction. It says nothing about verification — a deferred manual check lives in the build-order marker and `status.md`'s `Pending verification:` line — so a feature landing `🚧` carries it exactly as a `✓` one does, and the viewer reads a `🚧` owner as built only when its plan is stamped, since `🚧` alone also means mid-build or adopted. From v0.40 every plan `/b-feature` or `/b-module` completes carries the line; plans `/b-module` wrote before v0.40 did not, and the v0.40 migration stamps them.

Three conditions are mechanically detectable and the viewer reports all three. **The owner is discharged but the annotation survives** — a feature at `✓`, or a queue item ticked — so the discharge was missed and the annotation is now itself the false claim; a won't-fixed queue item is the same finding with a different repair, since the work will not happen and the claim it qualifies should go rather than be unmarked. **The owner cannot be resolved** — none named, a name without its module, a feature in no build order, or a queue item absent from that module's queue, which reads either as discharged-and-deleted or as never recorded and is actionable both ways. **The owner is absorbed** — a `⏸` entry carrying `Remaining: none`, whose scope an earlier feature took over entirely, so it will never build anything and the claim it owns is no longer pending; this is the pull-forward case above, and it is the only one of the three with no other reader, since the entry never reaches `✓` and its name resolves. Requiring an owner is what makes this audit possible at all: an exempt form would be a class of annotation for which the first check is structurally unreachable. Whether an *unannotated* prose claim is true of the code is not decidable from `docs/`, which is why the convention is a write-side rule with a read-side audit rather than a check.
