...
Every workflow gate presents findings and waits for explicit confirmation — an **operator gate**, bound per runtime in `_bower/framework.md` → *Runtime bindings* (Claude Code presents it through `AskUserQuestion`; Codex in the ordinary reply, ending the turn). The agent recommends; the engineer decides. This is a deliberate design choice — we're building with engineers who expect to be consulted on design decisions, not presented with fait accompli.
