...
2. **Spec ↔ code drift (bidirectional).** Either the implementation wandered from the feature's `plan.md` / the acceptance contract, **or** `plan.md` / `status.md` is stale against the code. Both are the map and the territory disagreeing. The direction matters for the resolution: stale doc → reconcile the doc; code violates an agreed criterion → a behavioural fix.
   ...
   **A claim annotated *decided, not built* is not drift** (`_bower/framework-reference.md` → *Forward-written claims*): it declares itself untrue of the code, so treat it as non-existent and never report it, in this dimension or any other.

   Its **lifecycle** is reportable for an annotation met in this module's feature plans or its `## Software architecture` entry, resolved against this module's `## Build order` only — the reviewer reads no other module's roster and no `findings.md`. Two cases:

   - **The owner is a feature in this module's build order that has landed** — `✓`, or `🚧` with its plan stamped `Confirmed`. Class `inline-reconcile`: delete the annotation. Where it sits in **that feature's own plan** it is *self-owned* and has two readings with opposite repairs — a run that finished and skipped the deletion (delete the annotation), or a run interrupted after the plan write (the code is absent; class it `status-fix`, the marker is wrong). Decide from the code and say which.
   - **The owner is missing or unqualified** — no `feature` named, or a bare name with no `<module>/`. Report as an **observation** naming file, line, claim and both repairs — the owner it would need, or deletion if nothing will ever build it. `/b-review` puts it to the operator at triage; accepted, it is `inline-reconcile` (the operator names an owner, or a stale claim in a plan is deleted) or `route:/b-design` (a stale claim in `architecture.md`).

   An owner in another module, and any `<module>/Q-<slug>` item, are out of scope: not reported as findings or observations. Resolve owners **qualified**; a bare name matching this module may mean another's.

   **The one edit the class licenses outside `plan.md`/`status.md`.** Where the stale annotation is in `docs/architecture.md`, an `inline-reconcile` may delete the annotation and nothing else in that file — the same narrow permission `/b-feature` Step 6's discharge sweep carries, and for the same reason: `/b-design` writes annotations there and no other command comes back for them. If deleting it would leave the surrounding prose *wrong* rather than merely unmarked, that is an architecture edit: class the finding `route:/b-design` instead.
