...
| ADR identity — `id: ADR-<slug>` in `<slug>.md` (v0.38+) or `id: ADR-NNNN` in `NNNN-*.md` (legacy); every `docs/adr/*.md` but `index.md` is an ADR; the route key is the token after `ADR-`; ordered by `date` then key | `framework-reference.md` → *ADRs* | which files are ADRs at all, `#/adr/<key>` routes, `adrRefs` in prose and links (both ID shapes, both link forms) |
