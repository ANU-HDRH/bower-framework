# Bower Framework Changes — Archive (v0.8 – v0.19)

Archived framework change entries, most recent first. These are the verbatim entries as originally written; nothing has been reworded or compacted.

**Why they live here.** `_bower/changes.md` is scaffolded into every Bower project and is read by `/b-upgrade` during an upgrade. Keeping every historical entry there made it a 120 KB file that no project needed and every upgrade paid to read. v0.20 (2026-07-17) is the boundary: it established the current framework architecture — delegated implementation, the slim `framework.md` router with `framework-reference.md` behind it, and ADR applicability metadata. Entries below v0.20 describe a framework that no longer has that shape.

This file is **not** copied into projects by the scaffold. It stays in the framework repo, which `/b-upgrade` clones anyway — so a project on a pre-v0.20 version can still reach these migration notes at `<clone>/docs/changes-archive.md`. `_bower/changes.md` keeps a version index covering these releases, so the head file still answers "what happened when" without carrying the bodies.

## Version index

**Migration** is the class of project-side work each version's notes required: *none* (no project changes), *mechanical* (direct file edits), or *judgement* (the model must read project content and synthesise).

| Version | Date | Summary | Migration |
| --- | --- | --- | --- |
| v0.19 | 2026-06-03 | `/b-index`: structure-preserving regeneration, not template clobber | none |
| v0.18 | 2026-05-29 | `/b-review`: fresh-eyes module review with a transient reconciliation plan | none |
| v0.17 | 2026-05-28 | Lightweight ADR shape: cut wordcount, keep umbrella scope | none |
| v0.16 | 2026-05-25 | `/b-feature`: write `plan.md` after the gate, before implementing | none |
| v0.15 | 2026-05-22 | Experience surface: `docs/ui.md` and `/b-ui` | judgement |
| v0.14 | 2026-05-19 | Ad-hoc reconcile convention; architecture-holding principle; default `settings.json` | mechanical |
| v0.13 | 2026-05-19 | Per-project version stamp; `/b-upgrade` skill | mechanical |
| v0.12 | 2026-05-19 | Framework guidance extracted to `_bower/framework.md`; scaffold script | mechanical |
| v0.11 | 2026-05-18 | `architecture.md` becomes a two-view document: runtime + software | mechanical |
| v0.10 | 2026-05-15 | Change brief and analyst subagent; `/b-design` becomes six stages | mechanical |
| v0.9 | 2026-05-05 | ADRs replace `design-decisions.md`; new `/b-adr` command | judgement |
| v0.8 | 2026-05-04 | Module-integration tests first-class; commands renamed to `/b-*` | mechanical |
