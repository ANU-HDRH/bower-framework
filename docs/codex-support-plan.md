...
### ✅ M4 — Release gate (DONE, uncommitted)

Landed as specified below. Three `-f`-guarded blocks after the viewer block, in order: adapter drift `--check`, adapter acceptance test, scaffold acceptance test. The two node ones warn-and-skip when node is absent; the scaffold test needs only bash and always runs. The header comment's abort list gained the new causes (`--help` output is unaffected — it prints lines 2–9 only).

Verification: full `--dry-run` **exit 0** with all four blocks green, under a temporary v0.33 bump (`_bower/VERSION`, viewer `SCHEMA_VERSION`, the three viewer-test fixture `_bower/VERSION` files, a stub `## v0.33` section) — reverted afterwards, tree back to baseline. Untampered on the real v0.32 state it reaches the pre-existing `tag v0.32 already exists` abort, which is downstream of all four gates. Tamper of one byte in a generated file (`.claude/commands/b-recap.md`, one letter of `description:`) → drift block names `stale: .claude/commands/b-recap.md` and aborts before the tag/notes stage. `PATH=/usr/bin:/bin` → three warnings, scaffold test still runs.

No changelog entry: release tooling is not framework behaviour downstream projects experience (contributor `CLAUDE.md`, "Releasing a framework version").
