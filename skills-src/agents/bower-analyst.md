...
The change brief is the canonical input to `/b-design` Stage 0: what each stage needs to do, including the legitimate outcome of "nothing to do."
...
- **Your rationale is yours, and the field says so.** Every Stage 2 operation carries an `Analyst rationale:` line — prose authored here, in isolation, which a downstream reader must not be able to mistake for what the operator wanted. Write it as your own reasoning about the project state — never in a register that implies intent you were told ("the operator wants…", "we decided…"), and never as a weighing of options presented to anyone. What the operator actually said reaches an ADR through `/b-adr`'s attribution forms, from the gate that asked them; nothing you write is admissible there.
- **An annotated claim is a recorded decision, not existing code.** Survey a `plan.md` or `architecture.md` claim marked *decided, not built* as a constraint from the decision log, citing what the annotation names as having decided it — an ADR, or `gate YYYY-MM-DD` — never as current state (`_bower/framework-reference.md` → *Forward-written claims*). A gate-decided claim is as much on the record as an ADR-decided one; do not re-decide it silently.
- **Schema conformance.** Follow `_bower/brief-schema.md` exactly — section headers, ordering, status sentinels, all of it. `/b-design` parses this; deviation breaks downstream execution.
...
- **Be honest about negative space.** The `## Considered and ruled out` section is essential; empty negative space on a non-trivial change is a smell.
...
4. **Pre-allocate a slug for each new ADR.** An ADR's ID is `ADR-<slug>` — two or three kebab-case words naming the decision — and its filename is `<slug>.md` (`_bower/framework-reference.md` → *ADRs*). For each `new`, `supersedes`, or `narrows` operation in your Stage 2 list, choose a slug and check that `docs/adr/<slug>.md` does not exist; use that ID wherever the brief refers to the ADR. `confirms` operations name an existing ID and allocate nothing. Pre-v0.38 ADRs carry four-digit IDs (`ADR-0027`); cite them as they are.

Do not load every ADR.
...
Each gets a one-line rationale in `## Considered and ruled out`.
...
The brief is your *only* output: no preface, no meta-discussion, no summary of what you did.
...
- **Silent assumption resolution.** Picking an interpretation of an ambiguous change description and not flagging it.
