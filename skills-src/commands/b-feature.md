...
10. **Check for a routed review finding — in every plan item 1's glob returned, not just this module's.** A review's findings stay in the *reviewed* module's plan even when the fix belongs elsewhere: `/b-review` writes `Run /b-feature modify billing <slug>` into `auth`'s plan and nothing is ever written on billing's side. So the plan that owns this invocation's finding is frequently a module you are not touching, and one whose `Review:` marker you have no reason to read. Read each returned plan's `## Findings` checklist and look for an open `route:/b-feature` item whose command matches this invocation. Match in this order:
    ...
    If the operator named a finding that is not in the plan they named, or is already disposed of, say so and ask rather than silently falling through to a slug match. If one matches, read its indented `Location:` / `Drift:` / `Resolution:` brief. **That brief is a primary input, not background.** It was written by `bower-reviewer` against the whole module at once — a view this command does not have — and the reviewer's context is gone. Treat `Drift:` as an observation to verify against the code, not as a conclusion to accept: if the code no longer shows it, say so in Step 2 rather than inventing work. Cite the finding ID **and the module whose plan holds it** in the proposal so the operator can tie it back, and name both in the Step 7 handoff — the review to resume is the one that *found* the drift, which on a cross-module finding is not the module you just changed. Do **not** tick the box yourself — `/b-review` owns the plan.
...
**The ledger always states the review-plan outcome**, in one line, even when there is nothing to report — `review plans: none open`, `review plans: auth (3 open) — none route here`, or `review plans: auth F6 loaded — routes into this module from auth's review`. Name the plans the glob found, not the module being changed: a bare "no open review" is the answer to a question nobody asked when the finding lives in another module's plan. This is the one ledger entry with no "deliberately skipped" form. Everything else in Step 1 is selective by design, so an absence is ambiguous; here an absence means the check did not happen, and the operator can say so at the gate before a proposal is built on a finding nobody read.
...
Pick exactly one — **except** when this change discharged a routed review finding (Step 1.10). Then the next move is `/b-review <reviewed-module>`, with the finding ID: the item is still open in that plan, only `/b-review` may tick it, and an unticked finding holds the review open indefinitely. The reviewed module is the one whose plan holds the finding, which for a cross-module finding is *not* the module you just changed — naming the changed module here sends the operator to a review that does not exist. Say which finding, so they can confirm it landed.
